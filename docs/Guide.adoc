...
* ShiroAnnotationHandlerService now uses shiros AuthenticatedAnnotationHandler and doesn't redirect to auth/login
...
`shiro-quick-start` is designed to get you up and running with shiro quickly. It basically runs `create-wildcard-realm` 
...
create-auth-controller creates a base `controllers/../AuthController.groovy` controller and an `controllers/../AuthInterceptor.groovy`
...
create-wildcard-realm creates a wildcard realm. See <<Create a Wildcard Realm>>.

=== create ldap realm

create-ldap-realm creates an LDAP realm. See <<Creating an LDAP realm>>.

=== create shiro controller interceptor

create-shiro-controller-interceptor creates a new `controllers/../[insert controller name here]Interceptor.groovy` with a
...
Can only be 'native' which uses the shiro native session manager "with sensible defaults". If not set you get
the servlet containers (Tomcat) session manager
...
To get started with a wildcard realm type `create-wildcard-realm` from your project directory. This will create a default
